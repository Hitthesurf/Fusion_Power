...
\section{Deriving the Linear System}
We wish to find a matrix $\mathbb{B}^{(G)}$ and vector $\mathbf{F}^{(G)}$ so that the solution $\mathbf{u}$ to $\mathbb{B}^{(G)} \mathbf{u} = \mathbf{F}^{(G)}$ contains the coefficients of are $u$ in the discretised space. We can use a local formulation appoach. Thus we calculate the local $\mathbb{B}^{(L)}$ and $\mathbf{F}^{(L)}$ and using a local to global map we add the results to the global system. Here is an algorithm to add the local system to the global system. (In this case $d=2$).
...
In these derivations of linear systems we will use a finite element with two basis function. But it is trivial how to introduce more basis functions. 
\section{Linear System for $(SP)$}
We now find
We will discrtise $u$ From (\ref{SP_w}) we substitute are approximation for $u$ on this finite element. 

\section{Linear System for $(MMAP)$}
