...
\title{The Title of My M.Sc.\ Dissertation}
\author{Mark Pearson}
...
\end{equation}
In the integral over $\Gamma_D$ $u$ can be replaced with $g$.

DEFs

It should be noted that these sets are easy to discretise.
...
\end{cases}
\end{equation}
We solve for trial function with test functions . 
...
We now introduce an example on a unit square where $\Gamma_D$ has zero boundary conditions. This is similar to example $3$ but example $3$ failed to satisfy $\mathbf{b}\cdot\mathbf{n}$ on $\Gamma_D$ this can be visually seen from Figure \ref{E3_VF} showing its vector field as $\mathbf{b}$ on $y=0$ and $y=1$ is not parallel to the boundary. The vector field for this example shown in Figure \ref{E4_VF} is parallel to the boundary on $\Gamma_D$. Additionally, this has open and closed field lines.
