...
This method excels for when $\varepsilon \approx 1$.
...
In these examples, we calculate $f$ from passing $u$ through the PDE. Additionally, in these example we take $A_{||}=1$ and $\mathbb{A}_{\perp} = \mathbb{I}$. Where $\mathbb{I}$ is the identity matrix. Also, for the stabilisation methods we take $\sigma=0.1$. But first, we briefly discuss the basic idea of how the finite element solver Firedrake \cite{Dragon} which is based on the package FEniCSx \cite{FEniCSx} calculates the solution. Also, we visualise data in ParaView \cite{Para}, Ipyvolume \cite{Ipyvolume} and Matplotlib \cite{Matplotlib}. 
...
\section{Conclusion}
In this thesis, we discussed multiple existing methods to solve anisotropic diffusion problems and propose a new method $(PF\_STAB)$ which is slightly better than the best current method $(MMAP\_STAB)$ for highly anisotropic diffusion problems. However, a value for $\sigma$ must be chosen and it is unclear what value it should take.

Moreover, methods $(PF)$ and $(MMAP)$ do not work on closed field lines and we have to find $\Gamma_{in}$. But we found for the $(MMAP)$ method we can use a modified version of $\mathcal{L}$ to get reasonable error on closed field lines more research is needed to know if the selected points on the streamlines have to form a continuous line.
...
From the numerical calculations from \cite{DN} the DN shows 
DN method shows promise but has a forced iteration scheme

We found no reason to use the $(AP)$ method when the $(MMAP)$ method produces the same error for fewer degrees of freedom. Additionally, a similar argument is made for the $(LM)$ method, for $\varepsilon \ll 1$ discretisations from $(MMAP)$ and $(LM)$ are almost identical. Since the $(LM)$ method fails to calculate acceptable error for larger $\varepsilon$ there is no reason to use the $(LM)$ method. This is the same for the stabilisation variants. 
...
We have shown it is possible to solve partial differential equations via the finite element method on a GPU cluster. On a sequential implementation the calculation of the linear system of equations from the integration over the finite elements will take longer than finding the solution to the linear system. However, with a paraelle implementation the bottlenecks become data commication and any parts parts . Thus with more funding and reasearch into parelle linear system solvers we can then use robust discretisations $(PF\_STAB)$ and $(MMAP\_STAB)$ to get an accurate solution to a highly anisotropic diffsion problem in a reasonable time. 

Further research can be done into finding better discretisations but it is unclear if better methods can be found. With research into better pareller solvers you will also get the benifit of other industies driving development for better hardware.  

Finally, 

Thus is can find method, we can then use the addition of GPU methods to solve and get splution fast.
important to take advantage of latest hardware

Finally, the code used for the methods, the examples and generating data can be found on GitHub at \cite{Hub}. This mean results can be easily reproduced. To generate the data in the graphs and tables we used the functions in "makePics.py" located at \cite{Hub}.
